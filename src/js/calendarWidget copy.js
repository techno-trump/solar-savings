...
function setFieldsAndSend(fields) {
	...
	new Promise((resolve, reject) => {
		...
		inputNodes.forEach(function(inputNode) {
			var dataQ = inputNode.getAttribute("data-q");
			if (typeof mapping[dataQ] === "boolean") {
				inputNode.checked = mapping[dataQ];
				inputNode.dispatchEvent(new Event("change"));
			} else if (dataQ === "phone") {
				inputNode.value = "";
				mapping[dataQ].split("").forEach(char => {
					inputNode.value += char;
					inputNode.dispatchEvent(new Event("input"));
				})
			} else {
				inputNode.value = mapping[dataQ];
				inputNode.dispatchEvent(new Event("input"));
			}
		});
